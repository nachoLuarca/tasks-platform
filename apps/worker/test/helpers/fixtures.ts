...
/** A throwaway account with a unique address, for the account-email tests. Never cleaned up, same as below. */
export async function createTestUser(): Promise<{ id: string; email: string; name: string }> {
  return prisma.user.create({
    data: { email: `account-${randomUUID()}@example.com`, passwordHash: 'not-a-real-hash', name: 'Account Test User' },
