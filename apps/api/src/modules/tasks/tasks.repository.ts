...
export const tasksRepository = {
   ...
   * it, in one transaction. `UPDATE ... SET "taskCounter" = "taskCounter" +
   * 1 ... RETURNING` takes a row lock on the project for the rest of the
   * transaction: a second, concurrent call for the same project blocks on
   * that UPDATE until the first transaction commits (or rolls back), so two
   * calls can never read the same counter value and hand out the same
   * number. This is why the increment and the read happen as a single
   * statement instead of a separate SELECT followed by an UPDATE.
   */
  async createWithNextNumber(input: CreateTaskInput): Promise<TaskEntity> {
    return prisma.$transaction(async (tx) => {
      const claimed = await tx.$queryRaw<{ taskCounter: number }[]>`
